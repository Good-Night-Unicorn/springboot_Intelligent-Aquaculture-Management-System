<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="采购时间" prop="caigoushijian">
				<el-date-picker
					:disabled="true  ||ro.caigoushijian"
			...
			<el-form-item class="add-item" label="采购说明" prop="caigoushuoming">
				<el-input v-model="ruleForm.caigoushuoming" 
					placeholder="采购说明" clearable :disabled=" false  ||ro.caigoushuoming"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='caigoushuoming'){
							this.ruleForm.caigoushuoming = obj[o];
							this.ro.caigoushuoming = true;
							continue;
						}
					}
			...
			// 多级联动参数
			info() {
				this.$http.get(`shebeicaigou/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(table+`/update`,obj).then(res => {});
						await this.$http.post(`shebeicaigou/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

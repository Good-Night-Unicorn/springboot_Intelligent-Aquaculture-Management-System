<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="采购单号" prop="caigoudanhao">
				<el-input v-model="ruleForm.caigoudanhao" placeholder="采购单号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="入库时间" prop="rukushijian">
				<el-date-picker
					:disabled="true  ||ro.rukushijian"
			...
			<el-form-item class="add-item" label="入库备注" prop="rukubeizhu">
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`caigoujilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(table+`/update`,obj).then(res => {});
						await this.$http.post(`caigoujilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="调拨地点" prop="diaobodidian">
				<el-date-picker
					:disabled="true  ||ro.diaobodidian"
					value-format="yyyy-MM-dd HH:mm:ss"
					v-model="ruleForm.diaobodidian" 
					type="datetime"
					placeholder="调拨地点">
				</el-date-picker>
			</el-form-item>
			<el-form-item class="add-item" label="调拨备注" prop="diaobobeizhu">
				<el-input v-model="ruleForm.diaobobeizhu" 
					placeholder="调拨备注" clearable :disabled=" false  ||ro.diaobobeizhu"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			...
			this.ruleForm.diaobodidian = this.getCurDateTime()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='diaobodidian'){
							this.ruleForm.diaobodidian = obj[o];
							this.ro.diaobodidian = true;
							continue;
						}
						if(o=='diaobobeizhu'){
							this.ruleForm.diaobobeizhu = obj[o];
							this.ro.diaobobeizhu = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`diaobojilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(table+`/update`,obj).then(res => {});
						await this.$http.post(`diaobojilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

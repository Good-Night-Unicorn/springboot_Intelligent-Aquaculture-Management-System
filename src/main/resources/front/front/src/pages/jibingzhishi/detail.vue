	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jibingzhishi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">疾病类型</div>
						<div class="text "  >{{detail.jibingleixing}}</div>
					</div>
					<div class="item">
						<div class="lable">疾病症状</div>
						<div class="text "  >{{detail.jibingzhengzhuang}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">防治建议</div>
						<div class="text "  >{{detail.fangzhijianyi}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jibingzhishi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jibingzhishi','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.jibingmingcheng;
						if(this.detail.jibingtupian) {
							this.detailBanner = this.detail.jibingtupian.split(",w").length>1?[this.detail.jibingtupian]:this.detail.jibingtupian.split(',');
						}
						this.$forceUpdate();
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('jibingzhishi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'jibingzhishi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jibingzhishi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussjibingzhishi/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussjibingzhishi/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussjibingzhishi/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussjibingzhishi/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jibingzhishiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此疾病知识？') .then(_ => {
					this.$http.post('jibingzhishi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {

	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jibingyujing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">预警时间</div>
						<div class="text "  >{{detail.yujingshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">水质异常</div>
						<div class="text "  >{{detail.shuizhiyichang}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jibingyujing','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jibingyujing','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('jibingyujing','疾病报告')" 
							@click="onAcross('jibingbaogao','','','','')" type="warning">
							疾病报告
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">预防措施</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.yufangcuoshi"></div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`jibingyujing`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jibingyujing', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jibingyujingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此疾病预警？') .then(_ => {
					this.$http.post('jibingyujing/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',

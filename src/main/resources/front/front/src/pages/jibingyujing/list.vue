			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-date-picker
						...
						type="daterange"
						range-separator="至"
						start-placeholder="预警时间起始"
						end-placeholder="预警时间结束"
				...
				<el-button class="list-add-btn" v-if="btnAuth('jibingyujing','新增')" type="primary" @click="add('/index/jibingyujingAdd')">
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.timeRange&&this.timeRange.length > 0) {
					searchWhere.yujingshijianstart = this.timeRange[0];
					searchWhere.yujingshijianend = this.timeRange[1];
				}
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`jibingyujing/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jibingyujingDetail', query: params});
			},

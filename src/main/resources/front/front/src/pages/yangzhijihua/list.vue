		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					<div class="lable">养殖区域：</div>
					<el-input v-model="formSearch.yangzhiquyu" placeholder="养殖区域" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					<i class="el-icon-search"></i>
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('yangzhijihua','新增')" type="primary" @click="add('/index/yangzhijihuaAdd')">
			...
			<div class="list">
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)">
						...
						<div class="infoBox">
							<div class="name">{{item.yangzhiquyu}}</div>
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/yangzhipinzhong/yangzhipinzhong').then(res => {
				if (res.data.code == 0) {
					this.yangzhipinzhongOptions = res.data.data;
					this.selectOptionsList.push({name:'养殖品种',list:this.yangzhipinzhongOptions,tableName: 'yangzhipinzhong',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.yangzhiquyu != '') searchWhere.yangzhiquyu = '%' + this.formSearch.yangzhiquyu + '%';
				if (this.formSearch.yangzhipinzhong != '') searchWhere.yangzhipinzhong = this.formSearch.yangzhipinzhong;
				if(!this.centerType){
					params['sfsh'] = '是';
				}
				...
				this.$http.get(`yangzhijihua/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/yangzhijihuaDetail', query: params});
			},

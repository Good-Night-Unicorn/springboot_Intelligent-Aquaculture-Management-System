	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yangzhijihua?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">养殖品种</div>
						<div class="text "  >{{detail.yangzhipinzhong}}</div>
					</div>
					<div class="item">
						<div class="lable">养殖数量</div>
						<div class="text "  >{{detail.yangzhishuliang}}</div>
					</div>
					<div class="item">
						<div class="lable">生长阶段</div>
						<div class="text "  >{{detail.shengzhangjieduan}}</div>
					</div>
					<div class="item">
						<div class="lable">投苗时间</div>
						<div class="text "  >{{detail.toumiaoshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">预计收获时间</div>
						<div class="text "  >{{detail.yujishouhuoshijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yangzhijihua','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yangzhijihua','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('yangzhijihua','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yangzhijihua', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yangzhijihuaAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此养殖计划？') .then(_ => {
					this.$http.post('yangzhijihua/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',

<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="存放位置" prop="cunfangweizhi">
				<el-input v-model="ruleForm.cunfangweizhi" 
					placeholder="存放位置" clearable :disabled=" false  ||ro.cunfangweizhi"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="物品描述" prop="wupinmiaoshu">
				<editor 
					v-model="ruleForm.wupinmiaoshu" 
					class="editor" 
					myQuillEditor="wupinmiaoshu"
					action="file/upload">
				</editor>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`wupinkucun/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`wupinkucun/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

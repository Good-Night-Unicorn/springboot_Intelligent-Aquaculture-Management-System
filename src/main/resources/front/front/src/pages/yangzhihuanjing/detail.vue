	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yangzhihuanjing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">pH值</div>
						<div class="text "  >{{detail.suanjiandu}}</div>
					</div>
					<div class="item">
						<div class="lable">溶解氧</div>
						<div class="text "  >{{detail.rongjieyang}}</div>
					</div>
					<div class="item">
						<div class="lable">氨氮</div>
						<div class="text "  >{{detail.andan}}</div>
					</div>
					<div class="item">
						<div class="lable">电导率</div>
						<div class="text "  >{{detail.diandaolv}}</div>
					</div>
					<div class="item">
						<div class="lable">浊度</div>
						<div class="text "  >{{detail.zhuodu}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yangzhihuanjing','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yangzhihuanjing','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('yangzhihuanjing','养殖计划')" 
							@click="onAcross('yangzhijihua','','','','')" type="warning">
							养殖计划
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('yangzhihuanjing','疾病预警')" 
							@click="onAcross('jibingyujing','','','','')" type="warning">
							疾病预警
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.yangzhiquyu;
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`yangzhihuanjing`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yangzhihuanjing', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yangzhihuanjingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此养殖环境？') .then(_ => {
					this.$http.post('yangzhihuanjing/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',

<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="报废时间" prop="baofeishijian">
				<el-date-picker
					:disabled="true  ||ro.baofeishijian"
			...
			<el-form-item class="add-item" label="报废原因" prop="baofeiyuanyin">
				<el-input v-model="ruleForm.baofeiyuanyin" 
					placeholder="报废原因" clearable :disabled=" false  ||ro.baofeiyuanyin"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='baofeiyuanyin'){
							this.ruleForm.baofeiyuanyin = obj[o];
							this.ro.baofeiyuanyin = true;
							continue;
						}
					}
			...
			// 多级联动参数
			info() {
				this.$http.get(`shebeibaofei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(table+`/update`,obj).then(res => {});
						await this.$http.post(`shebeibaofei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

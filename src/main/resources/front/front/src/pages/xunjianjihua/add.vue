<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="巡检周期" prop="xunjianzhouqi">
				<el-input v-model="ruleForm.xunjianzhouqi" 
					placeholder="巡检周期" clearable :disabled=" false  ||ro.xunjianzhouqi"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="巡检内容" prop="xunjianneirong">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="巡检内容"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`xunjianjihua/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`xunjianjihua/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="水质异常" prop="shuizhiyichang" >
					<el-input v-model="ruleForm.shuizhiyichang" placeholder="水质异常" clearable  :readonly="ro.shuizhiyichang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="水质异常" prop="shuizhiyichang" >
					<el-input v-model="ruleForm.shuizhiyichang" placeholder="水质异常" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-if="type!='info'"  label="预防措施" prop="yufangcuoshi" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.yufangcuoshi" 
					class="editor"
					myQuillEditor="yufangcuoshi"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yufangcuoshi" label="预防措施" prop="yufangcuoshi" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.yufangcuoshi"></span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='shuizhiyichang'){
							this.ruleForm.shuizhiyichang = obj[o];
							this.ro.shuizhiyichang = true;
							continue;
						}
						if(o=='jibingfengxian'){
						...
						if(o=='yufangcuoshi'){
							this.ruleForm.yufangcuoshi = obj[o];
							this.ro.yufangcuoshi = true;
							continue;
						}
					}
				}
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.yufangcuoshi = this.ruleForm.yufangcuoshi.replace(reg,'../../../springboot1z11jbg7/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jibingyujing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jibingyujingCrossAddOrUpdateFlag = false;
											this.parent.search();

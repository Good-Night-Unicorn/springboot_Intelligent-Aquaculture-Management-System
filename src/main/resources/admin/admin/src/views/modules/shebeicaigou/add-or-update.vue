	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-else-if="ruleForm.caigoudanhao" label="采购单号" prop="caigoudanhao" >
					<el-input v-model="ruleForm.caigoudanhao" placeholder="采购单号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="采购说明" prop="caigoushuoming" >
					<el-input v-model="ruleForm.caigoushuoming" placeholder="采购说明" clearable  :readonly="ro.caigoushuoming"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="采购说明" prop="caigoushuoming" >
					<el-input v-model="ruleForm.caigoushuoming" placeholder="采购说明" readonly></el-input>
				</el-form-item>
			</template>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shebeicaigou/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shebeicaigouCrossAddOrUpdateFlag = false;
											this.parent.search();

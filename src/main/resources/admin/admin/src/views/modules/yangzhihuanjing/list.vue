<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex"}' >
					<div :style='{"width":"30%","margin":"0 10px 0 0","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","alignItems":"center","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">养殖区域</label>
						<el-input v-model="searchForm.yangzhiquyu" placeholder="养殖区域" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yangzhihuanjing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('yangzhihuanjing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-quxiao13" :style='{"margin":"0 2px","fontSize":"14px","color":"#4AAACE","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yangzhihuanjing','环境数据分析')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('yangzhihuanjing','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yangzhihuanjing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yangzhihuanjing','养殖计划')" @click="yangzhijihuaCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								养殖计划
							</el-button>
							<el-button class="btn8" v-if="isAuth('yangzhihuanjing','疾病预警')" @click="jibingyujingCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								疾病预警
							</el-button>
							<el-button class="edit" v-if=" isAuth('yangzhihuanjing','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yangzhihuanjing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<yangzhijihua-cross-add-or-update v-if="yangzhijihuaCrossAddOrUpdateFlag" :parent="this" ref="yangzhijihuaCrossaddOrUpdate"></yangzhijihua-cross-add-or-update>
		<jibingyujing-cross-add-or-update v-if="jibingyujingCrossAddOrUpdateFlag" :parent="this" ref="jibingyujingCrossaddOrUpdate"></jibingyujing-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="yangzhiquyuChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import yangzhijihuaCrossAddOrUpdate from "../yangzhijihua/add-or-update";
	import jibingyujingCrossAddOrUpdate from "../jibingyujing/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				yangzhijihuaCrossAddOrUpdateFlag: false,
				jibingyujingCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			yangzhijihuaCrossAddOrUpdate,
			jibingyujingCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			yangzhijihuaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yangzhijihuaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yangzhihuanjing');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yangzhijihuaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yangzhijihuaCrossaddOrUpdate.init(row.id,type);
				});
			},
			jibingyujingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jibingyujingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yangzhihuanjing');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jibingyujingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jibingyujingCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var yangzhiquyuChart1 = echarts.init(document.getElementById("yangzhiquyuChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							yangzhiquyuChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								yangzhiquyuChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			init () {
				let urlwendu = 'yangzhihuanjing/remind/wendu/1';
				urlwendu+=`?remindend=5`,
				
				urlwendu+=`&remindstart=35`,
				...
				this.$http({
					...
					if (data && data.code == 0 && data.count>0 ) {
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `温度预警`),
								h('p', null, [
				...
				let urlsuanjiandu = 'yangzhihuanjing/remind/suanjiandu/1';
				urlsuanjiandu+=`?remindend=6.5`,
				
				urlsuanjiandu+=`&remindstart=8.5`,
				...
				this.$http({
					...
					if (data && data.code == 0 && data.count>0 ) {
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `pH值预警`),
								h('p', null, [
									h('div', { style: 'text-align: center;font-weight: bold;padding: 0 0 10px;font-size: 16px' }, `${data.count}条记录到达预警`),
									h('div', { style: 'text-align: center' }, [
										h('span', null, `包含：`),
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.yangzhiquyu!='' && this.searchForm.yangzhiquyu!=undefined){
					params['yangzhiquyu'] = '%' + this.searchForm.yangzhiquyu + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "yangzhihuanjing/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {

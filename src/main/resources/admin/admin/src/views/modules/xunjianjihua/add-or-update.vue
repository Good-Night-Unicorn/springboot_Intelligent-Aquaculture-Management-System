	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="巡检周期" prop="xunjianzhouqi" >
					<el-input v-model="ruleForm.xunjianzhouqi" placeholder="巡检周期" clearable  :readonly="ro.xunjianzhouqi"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="巡检周期" prop="xunjianzhouqi" >
					<el-input v-model="ruleForm.xunjianzhouqi" placeholder="巡检周期" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item class="textarea" v-if="type!='info'" label="巡检内容" prop="xunjianneirong" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="巡检内容"
					v-model="ruleForm.xunjianneirong" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.xunjianneirong" label="巡检内容" prop="xunjianneirong" >
				<span class="text">{{ruleForm.xunjianneirong}}</span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='xunjianzhouqi'){
							this.ruleForm.xunjianzhouqi = obj[o];
							this.ro.xunjianzhouqi = true;
							continue;
						}
						if(o=='xunjianneirong'){
							this.ruleForm.xunjianneirong = obj[o];
							this.ro.xunjianneirong = true;
							continue;
						}
					}
				}
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `xunjianjihua/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xunjianjihuaCrossAddOrUpdateFlag = false;
											this.parent.search();

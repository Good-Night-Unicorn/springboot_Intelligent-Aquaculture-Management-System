	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="存放位置" prop="cunfangweizhi" >
			...
			<el-form-item v-else-if="ruleForm.wupinmiaoshu" label="物品描述" prop="wupinmiaoshu" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.wupinmiaoshu"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.wupinmiaoshu = this.ruleForm.wupinmiaoshu.replace(reg,'../../../springboot1z11jbg7/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `wupinkucun/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.wupinkucunCrossAddOrUpdateFlag = false;
											this.parent.search();

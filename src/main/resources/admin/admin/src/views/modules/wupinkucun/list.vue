<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex"}' >
					<div :style='{"width":"30%","margin":"0 10px 0 0","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","alignItems":"center","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">物品名称</label>
						<el-input v-model="searchForm.wupinmingcheng" placeholder="物品名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"width":"30%","margin":"0 10px 0 0","alignItems":"center","display":"flex"}' class="select" label="物品类型" prop="wupinleixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","alignItems":"center","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">物品类型</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('wupinkucun','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('wupinkucun','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-quxiao13" :style='{"margin":"0 2px","fontSize":"14px","color":"#4AAACE","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('wupinkucun','物品库存统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				<el-table class="tables"
					:stripe='true'
					:style='{"padding":"10px 0","borderColor":"#eee","borderRadius":"0","borderWidth":"1px 0 0 1px","background":"none","width":"100%","fontSize":"inherit","borderStyle":"solid"}' 
					:border='true'
					v-if="isAuth('wupinkucun','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('wupinkucun','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('wupinkucun','采购记录')" @click="caigoujiluCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								采购记录
							</el-button>
							<el-button class="btn8" v-if="isAuth('wupinkucun','投喂日志')" @click="touweirizhiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								投喂日志
							</el-button>
							<el-button class="btn8" v-if="isAuth('wupinkucun','调拨记录')" @click="diaobojiluCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								调拨记录
							</el-button>
							<el-button class="btn8" v-if="isAuth('wupinkucun','库存盘点')" @click="kucunpandianCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								库存盘点
							</el-button>
							<el-button class="edit" v-if=" isAuth('wupinkucun','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('wupinkucun','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
			...
			<el-pagination
				...
				:style='{"padding":"0","boxShadow":"none","margin":"20px auto 0","whiteSpace":"nowrap","color":"#333","textAlign":"center","background":"#fff","width":"60%","fontSize":"inherit","fontWeight":"500"}'
			></el-pagination>
		</template>
		...
		<caigoujilu-cross-add-or-update v-if="caigoujiluCrossAddOrUpdateFlag" :parent="this" ref="caigoujiluCrossaddOrUpdate"></caigoujilu-cross-add-or-update>
		<touweirizhi-cross-add-or-update v-if="touweirizhiCrossAddOrUpdateFlag" :parent="this" ref="touweirizhiCrossaddOrUpdate"></touweirizhi-cross-add-or-update>
		<diaobojilu-cross-add-or-update v-if="diaobojiluCrossAddOrUpdateFlag" :parent="this" ref="diaobojiluCrossaddOrUpdate"></diaobojilu-cross-add-or-update>
...
<script>
	...
	import caigoujiluCrossAddOrUpdate from "../caigoujilu/add-or-update";
	import touweirizhiCrossAddOrUpdate from "../touweirizhi/add-or-update";
	import diaobojiluCrossAddOrUpdate from "../diaobojilu/add-or-update";
	...
	export default {
		data() {
			return {
				...
				caigoujiluCrossAddOrUpdateFlag: false,
				touweirizhiCrossAddOrUpdateFlag: false,
				diaobojiluCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			caigoujiluCrossAddOrUpdate,
			touweirizhiCrossAddOrUpdate,
			diaobojiluCrossAddOrUpdate,
		...
		methods: {
			...
			caigoujiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.caigoujiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','wupinkucun');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.caigoujiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.caigoujiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			touweirizhiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.touweirizhiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','wupinkucun');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.touweirizhiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.touweirizhiCrossaddOrUpdate.init(row.id,type);
				});
			},
			diaobojiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.diaobojiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','wupinkucun');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.diaobojiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.diaobojiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			kucunpandianCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kucunpandianCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
			...
			init () {
				this.wupinleixingOptions = "饲料,药品".split(',')
				let urlshuliang = 'wupinkucun/remind/shuliang/1';
				...
				this.$http({
					...
					if (data && data.code == 0 && data.count>0 ) {
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `数量预警`),
								h('p', null, [
...
<style lang="scss" scoped>
	...
	.main-content .el-pagination /deep/ .el-pager .number.active {
		...
		margin: 0 5px;
		color: #fff;
		background: linear-gradient( 270deg, #3C9AFF 0%, #5BFFE5 100%);
		display: inline-block;
		vertical-align: top;
		font-size: 16px;
		line-height: 48px;
